...
class Ubiquiti{
	...
	public function __construct($ip, $user, $password, $https = true, $port = '443', $timeout = 5, $version){
		$this->_ch	        = curl_init();
	...
	private function query($page, $timeout = false){
		...
		if($this->_version!="new") {
	   	 $postdata= ['username'=>$this->_username,'password'=>$this->_password,'redirect'=> $this->_baseurl,'uri'=> $page];
		 curl_setopt($this->_ch, CURLOPT_URL, $this->_baseurl.$page);
	        ...
	        } else { //LTU firmware nuevo
                 $postdata= ['username' =>$this->_username,'password'=>$this->_password];
